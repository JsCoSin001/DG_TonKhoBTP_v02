    ...
    internal static class NhapXuatVatTu_DB
    {
        ...
        private static string BuildSqlTinhTon(string excludeTenPhieu, int? excludeDanhSachMaSPId)
        {
            ...
            if (!string.IsNullOrWhiteSpace(excludeTenPhieu) && excludeDanhSachMaSPId.HasValue)
            {
                sql += @"
                  AND NOT (
                        lsxn.TenPhieu = @excludeTenPhieu
                        AND ttdh.DanhSachMaSP_ID = @excludeDanhSachMaSPId
                  )
                ";
            }
        ...
        private static DataTable TimDichVu(string keyword, CancellationToken cancellationToken)
        {
            string sql = @"
                SELECT
                    ...
                    '' AS donvi,
                    dsdh.MaDon AS MaDon,
                    ttdh.SoLuongMua AS SoLuongYeuCau,
                    0 AS SoLuongTon,
                    ttdh.DonGia AS DonGia
                FROM ThongTinDatHang ttdh
                INNER JOIN DanhSachDatHang dsdh
        ...
        private static void KiemTraNgayCapNhatTrongCungThang(NhapXuatVatTu_SaveModel model)
        {
            if (!model.OldNgay.HasValue)
                return;

            DateTime ngayCu = model.OldNgay.Value.Date;
            DateTime ngayMoi = model.Ngay.Date;

            if (ngayCu.Year != ngayMoi.Year || ngayCu.Month != ngayMoi.Month)
                throw new InvalidOperationException("Chỉ được phép sửa ngày trong cùng tháng của phiếu.");
        }

        private static void CapNhatNgayToanPhieuNeuCan(
            SQLiteConnection conn,
            SQLiteTransaction tran,
            NhapXuatVatTu_SaveModel model)
        {
            if (!model.OldNgay.HasValue || model.OldNgay.Value.Date == model.Ngay.Date)
                return;

            string tenPhieu = string.IsNullOrWhiteSpace(model.OldTenPhieu)
                ? model.TenPhieu
                : model.OldTenPhieu;

            if (string.IsNullOrWhiteSpace(tenPhieu) || string.IsNullOrWhiteSpace(model.NguoiLam))
                throw new InvalidOperationException("Không xác định được phiếu cần cập nhật ngày.");

            if (PhieuCoDongKhongChoSua(conn, tran, tenPhieu, model.NguoiLam))
                throw new InvalidOperationException("Không thể đổi ngày vì phiếu có dòng CanEdit = 0.");
            ...
            using (var cmd = new SQLiteCommand(sql, conn, tran))
            {
                ...
                cmd.Parameters.AddWithValue("@nguoiLam", model.NguoiLam.Trim());
                cmd.ExecuteNonQuery();
            }
        }

        private static bool PhieuCoDongKhongChoSua(
            SQLiteConnection conn,
            SQLiteTransaction tran,
            string tenPhieu,
            string nguoiLam)
        ...
        private static void CapNhatPhieuXuatGop(SQLiteConnection conn, SQLiteTransaction tran, NhapXuatVatTu_SaveModel model)
        {
            ...
            if (NhomXuatCoDongKhongChoSua(conn, tran, model.OldTenPhieu, model.OldDanhSachMaSPId.Value, model.NguoiLam))
                throw new InvalidOperationException("Phiếu này không được phép sửa vì CanEdit = 0.");

            XoaNhomXuat(conn, tran, model.OldTenPhieu, model.OldDanhSachMaSPId.Value, model.NguoiLam);
            InsertPhanBoXuat(conn, tran, model);
        }
        ...
        private static void InsertPhanBoXuat(SQLiteConnection conn, SQLiteTransaction tran, NhapXuatVatTu_SaveModel model)
        {
            ...
            if (!model.DanhSachKhoId.HasValue)
                throw new InvalidOperationException("Chưa chọn kho hàng để lưu phiếu.");
            ...
            foreach (DataRow row in rows.Rows)
            {
                ...
                // Dòng xuất vẫn lưu model.DanhSachKhoId,
                // nhưng kho không dùng để tính tồn.
            ...
            string nguoiLam)
        {
            string sql = @"
                SELECT
                    ...
                    NULL AS ThongTinDatHang_ID,
                    dms.id AS DanhSachMaSP_ID,
                    MAX(lsxn.DanhSachKho_ID) AS DanhSachKho_ID,
                    MAX(kho.TenKho) AS TenKho,
